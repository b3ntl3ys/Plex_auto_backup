...
class FolderZipper(QWidget):
    ...
    def kill_task_by_name(self,task_name):
        for proc in psutil.process_iter(['pid', 'name']):
            if proc.info['name'] == task_name:
                proc.kill()

    def zip_thread_finished(self):
        subprocess.Popen(['C:\Program Files\Plex\Plex Media Server\Plex Media Server.exe'])
    ...
    def start_zip_thread(self):        
        ...
        self.kill_task_by_name("Plex Media Server.exe")
        ...
        subprocess.check_output(['reg', 'export', registry_key, os.path.join(os.path.dirname(self.save_path), 'PlexMediaServer'+current_datetime_str+'.reg')])
    ...
    def update_countdown(self):
        ...
        if time_left.total_seconds() <= 0:
            ...
            self.next_zip_time = datetime.now() + timedelta(days=self.interval)
            self.countdown_timer.start(1000)
    def export_reg_finished(self):
        QMessageBox.information(self, 'Export Registry Key', 'Registry key exported successfully.')
